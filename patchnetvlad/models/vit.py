...
from timm.layers import Mlp, DropPath, trunc_normal_, resample_patch_embed, \
    resample_abs_pos_embed, PatchDropout, use_fused_attn
from timm.models._manipulate import named_apply, checkpoint_seq, adapt_input_conv
from timm.models._builder import build_model_with_cfg

from .patch_embed import PatchEmbed, ConvEmbed, MiniEncoder, HybridEmbed
...
class ResPostBlock(nn.Module):
    '''
        attention block with post normalization module
    '''
    def __init__(
            self,
            dim,
...
class VisionTransformer(nn.Module):
    ...
    def forward_head(self, x, pre_logits: bool = False):
        '''
            get the feature vector for classification
            Return B C
        '''
        if self.global_pool:
    ...
    def forward_global_feat(self, x, feat_type:str=''):
        '''
            get the global feature for retrieval
            Input B N(num_patches) C
            Return B C
        '''
        if feat_type == 'GAP':
...
def resize_pos_embed(
    ...
    *DEPRECATED* This function is being deprecated in favour of resample_abs_pos_embed
    ...
    assert len(gs_new) >= 2
    # print_log(f'Resized position embedding: {posemb.shape} ({[gs_old, gs_old]}) to {posemb_new.shape} ({gs_new}).')
...
def checkpoint_filter_fn(
    ...
    if 'visual.class_embedding' in state_dict or \
        "mask_token" in state_dict or \
        "encoder" in state_dict:
        return NotImplementedError('No implementation of this type of model')
...
class VisionTransformerEncoder(VisionTransformer):
    '''
        Vanilla vit with different patichy
    '''
    @classmethod
    def from_config(cls, from_pretrained=False):
        '''
            Args:
        ...
        visual_encoder = build_model_with_cfg(
            ...
            from_pretrained,
            pretrained_filter_fn=checkpoint_filter_fn, # filter the parameters
            **dict(model_args),
        )       
